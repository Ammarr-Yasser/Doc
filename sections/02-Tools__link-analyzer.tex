...
Some UI screenshots in this chapter reflect earlier prototype naming; in this report the tool is referred to as Link Analyzer.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/fig-link-analyzer-decision-flow}
  \caption{Link Analyzer decision flow}
  \label{fig:link-analyzer-decision-flow}
\end{figure}
\FloatBarrier

As shown in Figure~\ref{fig:link-analyzer-decision-flow}, Link Analyzer execution follows a fixed sequence from validated input to risk output.
...
Figure~\ref{fig:link-analyzer-endpoints} visualizes the endpoint grouping used by this chapter.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth]{figures/fig-link-analyzer-endpoints}
  \caption{Link Analyzer API endpoints}
  \label{fig:link-analyzer-endpoints}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/fig-link-analyzer-processing-pipeline}
  \caption{Link Analyzer processing pipeline}
  \label{fig:link-analyzer-processing-pipeline}
\end{figure}
\FloatBarrier
...
Figure~\ref{fig:link-analyzer-processing-pipeline} summarizes the execution stages used after input validation.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\linewidth,height=0.8\textheight,keepaspectratio]{figures/fig-link-analyzer-redirect-reputation}
  \caption{Link Analyzer redirect and reputation pipeline}
  \label{fig:link-analyzer-redirect-reputation}
\end{figure}
\FloatBarrier

Figure~\ref{fig:link-analyzer-redirect-reputation} highlights how redirect and reputation signals are combined before final scoring.
...
\item Troubleshooting steps: check API keys, check upstream service status, inspect validation failures, and review history entries.
\end{itemize}
