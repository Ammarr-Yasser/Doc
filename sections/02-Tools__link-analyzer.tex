...
\item URL normalization details: TBD (verify in code).
\item Size and payload limits for request body: TBD (verify in code).
...
\item Indicator flags and metadata fields: exact schema TBD (verify in code).
\end{itemize}
...
\item External API timeout or failure handling policy: TBD (verify in code).
...
\item Sensitive payload retention policy details: TBD (verify in code).
...
\item Score combination logic is rule-based/weighted in practice, but exact weights and thresholds are TBD (verify in code).
\end{itemize}
...
\item Rate limiting and abuse controls at gateway/API level: TBD (verify in code).
...
\item Stored versus transient fields in history are deployment-dependent: TBD (verify in code).
