...
\begin{figure}[H]
  ...
  \texttt{URL Input} $\rightarrow$ \texttt{Validation} $\rightarrow$ \texttt{Checks} $\rightarrow$ \texttt{Aggregation} $\rightarrow$ \texttt{Risk Output}
  }}
  \caption{Link Analyzer Decision Pipeline}
  \label{fig:link-analyzer-decision-pipeline}
\end{figure}
\FloatBarrier

As shown in Figure~\ref{fig:link-analyzer-decision-pipeline}, Link Analyzer execution follows a fixed sequence from validated input to risk output.
...
\begin{figure}[H]
  ...
  \caption{Link Analyzer: External intelligence inputs and resulting tool output indicators in the system architecture context. Diagram contains legacy prototype labels; the final scope of this report excludes email tooling.}
...
\begin{lstlisting}
URL Input -> Validation -> Checks -> Aggregation -> Risk Output
\end{lstlisting}
