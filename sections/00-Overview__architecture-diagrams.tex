...
System context is summarized in Figure~\ref{fig:system-context}, which shows the browser SPA communicating with the backend API over HTTPS. External intelligence providers are reached only by the backend, not directly by the frontend. The database is accessed by the backend for user state, history, and counters.
...
Figure~\ref{fig:system-architecture-overview} summarizes the high-level DefendX architecture and core service interactions.
