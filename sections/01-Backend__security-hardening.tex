...
\item
  Global CORS allows all origins (\texttt{*}) for other routes. This setting is development-friendly and should be restricted in production deployments.
\end{itemize}
...
\hypertarget{security-gaps}{%
\subsubsection{Security Gaps}\label{sec:01-Backend__security-hardening:security-gaps}}

\begin{itemize}
\tightlist
\item
  Rate limiting and abuse prevention are not evident in code (Not evidenced in the reviewed codebase).
\item
  CSRF protection and secure cookie strategy are not used because tokens are stored in \texttt{localStorage} (risk: XSS). Not evidenced in the reviewed codebase.
\item
  Production TLS termination and WAF configuration are not defined in code (Not evidenced in the reviewed codebase).
\end{itemize}
...
\item
  No raw URL/file content persistence beyond necessary metadata
\item
  No file execution
\item
