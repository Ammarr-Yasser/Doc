...
Figure~\ref{fig:system-context} presents the system context, including external intelligence sources and service boundaries.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig-system-context}
  \caption{System context and external intelligence sources}
...
\item
  Data Store: SQLAlchemy models backed by SQLite for \texttt{dev.db} and \texttt{test.db} (production database is deployment-dependent and configured by the operator).
\item
  Async Tasks: Celery worker factory wired to Flask config (\texttt{CELERY\_BROKER\_URL}, \texttt{CELERY\_RESULT\_BACKEND}).
\end{itemize}

Figure~\ref{fig:system-architecture-overview} shows the high-level DefendX architecture and core service interactions.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{figures/fig-system-architecture-overview}
  \caption{DefendX system architecture overview}
