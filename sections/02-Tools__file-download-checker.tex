...
\item File size limits are enforced, exact thresholds TBD (verify in code).
\item Allowed/restricted file types list: TBD (verify in code).
...
\item Exact response schema and optional fields: TBD (verify in code).
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{File Download Checker: Hashing and Reputation Lookup}
  \label{fig:file-checker-hashing-reputation-lookup}
\end{figure}
\FloatBarrier
...
\item External reputation API failure behavior: TBD (verify in code).
\item Temporary file cleanup failure handling: TBD (verify in code).
\end{itemize}
...
\item Logged metadata fields are deployment-dependent: TBD (verify in code).
\end{itemize}
...
\item Scoring appears rule-based with thresholds; exact weighting and cutoffs are TBD (verify in code).
\end{itemize}
...
\item Abuse controls such as rate limits and per-user quotas: TBD (verify in code).
...
\item Troubleshooting steps: verify file limits, inspect temp storage health, confirm API key validity, and review history entries.
\end{itemize}
